...
public class Wiley {
	public static Article[] getInformation(Article[] temp) throws Exception{
		...
		for(int i=0;i<temp.length;i++){
			...
			if (null != isconnect(temp[i].getUrl())){
				...
				institution = JsoupTest.InstitutionWiley(html);
				keyword = JsoupTest.KeywordWiley(html);
	...
	public static Article[] setUrl() throws IOException, InterruptedException{
		...
		for(int i=0;i<a.length;i++){
			if(a[i].getUrl().contains("/10.1002/")){
				urlList.add(a[i]);
			}
		}
	...
	public static void main(String[] args) throws Exception{
		...
		String outpath = "C:/Users/Roc/Desktop/Wiley.txt";
